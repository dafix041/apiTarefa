...
public class TarefaController : ControllerBase
{
    ...
    [HttpGet]
    [Route("listarConcluido")]
    public IActionResult ListarConcluido()
    {
        try
        {
            List<Tarefa> tarefas = _context.Tarefas.Where(x => x.Status=="concluido").ToList();
            return Ok(tarefas);
        }
        catch (Exception e)
        {
   ...
   [HttpGet]
    ...
    public IActionResult ListarNaoConcluidas()
    {
        try
        {
            List<Tarefa> tarefas = _context.Tarefas.Where(x => x.Status=="não concluido").ToList();
            return Ok(tarefas);
        }
        catch (Exception e)
        {
